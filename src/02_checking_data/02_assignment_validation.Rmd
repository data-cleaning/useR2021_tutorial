...
Variable checks are checks that can be performed on a field-by-field basis. 
For example checking if `Age` is non-negative, or of integer
type. Variable checks are among the simplest checks.

We will use the `SBS2000` data set, that is included with `validate`.

Load the `SBS2000` data set and look at the description:
...
numeric. When confronted with a data set (e.g. `SBS2000`) we see that this is the 
case.
...
c) `validate` also for conditional statements: `if {rule_p} rule_q`:
...
Use the `label` function to set the label of the third rule to "profit def".
Use `print(rules)` to see the differences.
